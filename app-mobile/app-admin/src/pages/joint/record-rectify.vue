  ...
  <view class="page">
    ...
    <view class="reject-banner" v-if="latestRejectLog">
      <view class="reject-header">
        ...
        <text class="reject-time">{{ formatTime(latestRejectLog.created_at || latestRejectLog.audited_at) }}</text>
      </view>
...
const loadDetail = async (id: string) => {
  ...
  const rejects = logs.filter(log => String(log?.action || '').toUpperCase() === 'REJECT')
  if (rejects.length > 0) {
    latestRejectLog.value = rejects[rejects.length - 1]
    return
  }
  latestRejectLog.value = null
}
...
const handleComplete = async () => {
  ...
  try {
    const result = await inspectionApi.auditJointTask(taskId.value, {
    ...
    setTimeout(() => {
      ...
      if (resolvedStatus === 'COMPLETED') {
        uni.redirectTo({ url: `/pages/joint/record-completed?id=${encodeURIComponent(taskId.value)}` })
        return
      }
      if (resolvedStatus === 'REJECTED') {
        uni.redirectTo({ url: `/pages/joint/record-rectify?id=${encodeURIComponent(taskId.value)}` })
        return
      }
