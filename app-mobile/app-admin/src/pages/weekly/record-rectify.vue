      ...
      <scroll-view class="content-area" scroll-y>
        ...
        <view class="question-item" v-for="(item, index) in currentCategoryItems" :key="item.item_id || index">
          ...
          <view class="feedback-box" v-if="Number(item.score_given) < Number(item.total_score)">
            ...
            <view class="audit-action-group" v-if="showAuditActions">
...
const showAuditActions = computed(() => {
  const status = `${taskInfo.value.status || ''}`.toUpperCase()
  return status === 'SUBMITTED' || status === 'RECTIFIED'
})
...
const loadDetail = async (id: string) => {
  const data = await inspectionApi.getWeeklyTaskDetail(id)
  ...
  majorItems.forEach((major) => {
    ;(major.minor_items || []).forEach((minor) => {
      merged.push({
        ...minor,
        score_given: minor.score_given,
...
const handleComplete = async () => {
  ...
  const missingAudit = allMinorItems.value.find(
    (item) => Number(item.score_given) < Number(item.total_score) && !itemAuditStatus.value[item.item_id]
  )
  if (missingAudit) {
  ...
  try {
    const result = await inspectionApi.auditWeeklyTask(taskId.value, {
      auditor_id: auditorId,
      action: finalAction,
      opinion: auditOpinion.value.trim() || (finalAction === 'PASS' ? '整改通过' : '整改驳回')
    })
    const nextStatus = String(result?.status || '').toUpperCase()
    const resolvedStatus = nextStatus || (finalAction === 'REJECT' ? 'REJECTED' : 'COMPLETED')
    uni.showToast({ title: '审核完成', icon: 'success' })
    setTimeout(() => {
      if (resolvedStatus === 'COMPLETED') {
        uni.redirectTo({ url: `/pages/weekly/record-completed?id=${encodeURIComponent(taskId.value)}` })
        return
      }
      if (resolvedStatus === 'REJECTED') {
        uni.redirectTo({ url: `/pages/weekly/record-rectify?id=${encodeURIComponent(taskId.value)}` })
        return
      }
