<template>
  <view class="page">
    ...
    <scroll-view scroll-y class="list-container">
      ...
      >
        ...
        <view class="card-content">
          <view class="card-header">
            ...
            <text class="card-progress" v-if="item.progress">{{ item.progress }}</text>
          ...
          <view class="card-info">
            <view class="info-row">
              <view class="user-info" v-if="item.submitter">
                <view class="avatar">👤</view>
                <text class="username">{{ item.submitter }}提交</text>
              </view>
              <view class="user-info" v-else>
                <text class="username">未提交</text>
              </view>
      ...
      <view v-if="listData.length === 0" class="empty-tip">
        <text>暂无数据</text>
      </view>
    </scroll-view>
...
import { fetchDailyTaskList } from '@/common/daily-control'
...
const loadTasks = async () => {
  loading.value = true
  try {
    const res: any = await fetchDailyTaskList({
    ...
    listData.value = list.map((item: any) => ({
      ...
      progress: item.completion_progress || '',
      status: item.status,
      submitter: item.submitter_name || '',
      time: item.submission_date || ''
    }))
  } catch (e) {
...
const handleStartNew = () => {
  uni.navigateTo({
    url: '/pages/executor/sop/daily-control-form-submitted/daily-control-form-submitted'
  })
}
...
const handleItemClick = (item: any) => {
  if (item.status === 'PENDING') {
    // 待检查 -> 提交页面
    uni.navigateTo({
      url: `/pages/executor/sop/daily-control-form-submitted/daily-control-form-submitted?id=${item.id}`
    })
  } else if (item.status === 'REJECTED') {
    // 待整改 -> 整改页面
    uni.navigateTo({
      url: `/pages/executor/sop/daily-control-form-submitted/daily-control-rectification/daily-control-rectification?id=${item.id}`
    })
  } else if (item.status === 'COMPLETED') {
    // 已完成 -> 专门的已完成详情页
    uni.navigateTo({
      url: `/pages/executor/sop/daily-control-form-submitted/daily-control-completed?id=${item.id}`
    })
  }
}
</script>
