...
import { submitDailyTask, fetchDailyTaskDetail } from '@/common/daily-control'
...
onLoad(async (options: any) => {
  ...
  const id = options?.taskId || options?.id
  if (id) {
    taskId.value = decodeURIComponent(id)
...
const loadDetail = async (id: string) => {
  loading.value = true
  try {
    const detailRes = await fetchDailyTaskDetail(id)
    const taskInfo = detailRes.task_info || detailRes.data?.task_info || detailRes
    const snapshot = detailRes.form_snapshot || detailRes.data?.form_snapshot || taskInfo.form_snapshot || []
...
const submitChecklist = async () => {
  ...
  try {
    await submitDailyTask(taskId.value, {
