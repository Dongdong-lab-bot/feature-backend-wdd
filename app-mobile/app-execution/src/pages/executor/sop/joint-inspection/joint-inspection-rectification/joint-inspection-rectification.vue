<template>
  <view class="page">
    ...
    <view class="header-card">
      <view class="header-title">{{ canteenName }}联合巡检表</view>
...
import { fetchJointTaskDetail, rectifyJointTask, type JointRectifyItem } from '@/common/joint-inspection'
...
const loadTaskDetail = async (id: string) => {
  try {
    const res: any = await fetchJointTaskDetail(id)
    ...
    console.error('Failed to load joint task detail:', e)
...
const handleSubmit = async () => {
  ...
  const feedbackPerItem: JointRectifyItem[] = rectifyList.value
  ...
  try {
    ...
    await rectifyJointTask(taskId.value, { feedbackPerItem, rectifierId })
    uni.hideLoading()
    uni.showToast({
      title: '整改反馈成功',
      icon: 'success'
