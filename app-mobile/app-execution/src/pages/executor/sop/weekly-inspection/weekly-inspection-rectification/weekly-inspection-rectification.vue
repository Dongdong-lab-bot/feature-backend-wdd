...
import { fetchWeeklyTaskDetail, rectifyWeeklyTask } from '@/common/weekly-inspection'
...
const loadTaskDetail = async (id: string) => {
  try {
    const res: any = await fetchWeeklyTaskDetail(id)
    ...
    majorItems.forEach((major: any) => {
      ...
      items.forEach((minor: any) => {
        // 针对周排查（打分制），提取不合格项：
        // 如果后端显式传了 is_qualified = false，或者 score_given 小于 total_score，都认为需要整改
    ...
    rectifyList.value = minorItems.map((item: any, index: number) => ({
      ...
      issueDesc: item.inspection_description || item.description || '',
    ...
    console.error('Failed to load weekly task detail:', e)
...
const handleSubmit = async () => {
  ...
  const feedbackPerItem: any[] = rectifyList.value
  ...
  try {
    ...
    await rectifyWeeklyTask(taskId.value, { feedback_per_item: feedbackPerItem, rectifier_id: rectifierId })
    uni.hideLoading()
    uni.showToast({
      title: '整改反馈成功',
      icon: 'success'
